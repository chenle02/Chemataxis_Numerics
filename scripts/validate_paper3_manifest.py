...
DIAGNOSTICS_DIR = ROOT / "docs" / "diagnostics"
...
def validate_diagnostics_pages(provenance: list[dict[str, Any]]) -> int:
    if not DIAGNOSTICS_DIR.exists():
        return 0
    pages = sorted(DIAGNOSTICS_DIR.glob("*.md"))
    require(bool(pages), "diagnostics directory exists but holds no pages")
    for page_path in pages:
        context = f"diagnostics page {page_path.name}"
        body = page_path.read_text(encoding="utf-8")
        require(
            "generated by scripts/build_diagnostics_index.py" in body,
            f"{context} is not marked as generated",
        )
        require(
            "not quantitative evidence" in body,
            f"{context} lacks the non-quantitative banner",
        )
        for entry in provenance:
            archive = entry["raw_path"]
            require(
                archive not in body and archive.removeprefix("docs/results/") not in body,
                f"{context} exposes provenance-only {entry['id']}",
            )
    return len(pages)
...
def main() -> int:
    try:
        ...
        diagnostics_pages = validate_diagnostics_pages(provenance)
    ...
    print(
        ...
        f"{diagnostics_pages} diagnostics pages."
    )
    return 0
